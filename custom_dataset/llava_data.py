...
class MMMUDataset(Dataset):
    def __init__(self, 
        ...
        tot_idx = 0
        dataset_map = {}
        for dataset_idx, category in enumerate(categories):
            datasets.append(load_dataset("MMMU/MMMU", category)[split])
            for idx in range(len(datasets[-1])):
                dataset_map[tot_idx + idx] = (dataset_idx, idx)
            tot_idx += len(datasets[-1])
    ...
    def __getitem__(self, idx):
        dataset_idx, idx = self.dataset_map[idx]
        x = self.datasets[dataset_idx][idx]

        question = x['question']
        
        for i in range(7):
            question = question.replace(f'<image {i+1}>', '')
...
class COCODataset(Dataset):
    def __init__(self, 
        ...
        # coco 
        fpath = os.path.join(self.root, f'/mnt/bn/us-aigc-temp/henry/clipscore/data/ref.json')
        with open(fpath, 'r') as f:
            self.annotations = json.load(f)

        self.images = []
        for x in self.annotations:
            self.images.append((x, self.annotations[x]))
    ...
    def __getitem__(self, idx):
        image_id, answers = self.images[idx]
        ...
        return_dict = {
            ...
            'answers': [{'answer': answer, 'confidence': 'yes', 'answer_id': i + 1} for i, answer in enumerate(answers)],
            'image': image,
            'captions': answers
        }
        return return_dict
...
class LlavaDataset:
    def __init__(
        ...
        if add_mmmu:
            mmmu = MMMUDataset(split=split)
            self.datasets.append(mmmu)
            self.dataset_dict['mmmu'] = mmmu
    ...
    def get_image_and_text(self, dataset, idx):
        if dataset == 'llava':
            ...
            if isinstance(dataset, VizWizDataset):
                ...
                idx = np.random.randint(len(dataset.prompts))
                human += f'{dataset.prompts[idx]}'
            elif isinstance(dataset, MMMUDataset):
                pass
            else:
    ...
    def _getitem(self, idx, dataset=None):
        ...
        if self.tokenizer is not None:
            if self.reverse:
                ...
                preamble = gpt = self.tokenizer(gpt, max_length=self.max_length, truncation=True, return_tensors='pt')

                input_ids = torch.cat([preamble['input_ids'][:, :-1], postamble['input_ids']], dim=1)[0, :self.max_length]
                attention_mask = torch.cat([preamble['attention_mask'][:, :-1], postamble['attention_mask']], dim=1)[0, :self.max_length]

                label_mask = torch.cat([torch.ones_like(preamble['attention_mask'][:, :-1]), torch.zeros_like(postamble['attention_mask'])], dim=1)[0, :self.max_length]
